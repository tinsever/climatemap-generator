...
_CLIMATE_ZONE_LINES = [
    ...
    (SUBTROPIC_MAX_LAT,  "Subtropen-Grenze",  "Subtropen-Grenze",          "4,6",   "#90e070", 0.45),
    (POLAR_CIRCLE_LAT,   "Nördlicher Polarkreis", "Südlicher Polarkreis",  "3,7",   "#80c8ff", 0.55),
]

_SST_LEGEND_LABELS: list[tuple[float, str]] = [
    (0.0,  "28"),
    (11.25, "24"),
    (22.5, "20"),
    (33.75, "16"),
...
_LAND_LEGEND_ITEMS: list[tuple[str, str]] = [
    ("tropical_wet",  "Regenwald"),
    ("tropical_dry",  "Savanne"),
    ("desert_hot",    "Wüste"),
    ("steppe",        "Steppe"),
    ("mediterranean", "Mediterran"),
    ("humid_sub",     "Feuchte Sub."),
    ("oceanic",       "Ozeanisch"),
...
def append_currents_layer(
    ...
    for x1, y1, x2, y2, stroke_w in arrows:
        ...
        pts = [
            ...
            (bx + px * shaft_half, by + py * shaft_half),
...
def append_climate_zone_lines(root: ET.Element, vb: ViewBox) -> None:
    g = ET.SubElement(
        root,
        _qname("g"),
        {"id": "WAZ_Klimazonen", "font-family": "sans-serif"},
    )
...
def append_title(
    ...
    ET.SubElement(
        g, _qname("rect"),
        {
            "x": f"{evb.min_x:.3f}", "y": f"{evb.min_y:.3f}",
            "width": f"{evb.width:.3f}", "height": f"{pad_h:.3f}",
    ...
    ET.SubElement(
        ...
        {
            "x": f"{cx:.3f}", "y": f"{cy:.3f}",
            "font-family": "sans-serif", "font-size": f"{fs:.3f}",
            "font-weight": "bold", "fill": "#ffffff", "fill-opacity": "0.95",
...
def _append_sst_legend_block(
    root: ET.Element, evb: ViewBox, defs: ET.Element, y: float, block_h: float
) -> None:
    bar_w     = evb.width * 0.65
    bar_x     = evb.min_x + (evb.width - bar_w) / 2.0
    heading_fs = block_h * 0.20
    bar_h     = block_h * 0.32
    ...
    ET.SubElement(
        g, _qname("text"),
        {
            "x": f"{bar_x + bar_w / 2:.3f}", "y": f"{y + heading_fs:.3f}",
            "font-family": _LEGEND_FONT, "font-size": f"{heading_fs:.3f}",
            "font-weight": "normal", "fill": _LEGEND_FILL,
            "text-anchor": "middle",
        },
    ).text = "Meeresoberflächentemperatur in °C"

    grad = ET.SubElement(
        defs, _qname("linearGradient"),
        {"id": grad_id, "x1": "0", "y1": "0", "x2": "1", "y2": "0",
         "gradientUnits": "objectBoundingBox"},
    )
    for lat, c in SST_STOPS:
        ET.SubElement(grad, _qname("stop"),
                      {"offset": f"{lat / 90.0 * 100.0:.1f}%",
                       "stop-color": rgb_to_hex(*c), "stop-opacity": "1"})

    ET.SubElement(
        g, _qname("rect"),
        {
            "x": f"{bar_x:.3f}", "y": f"{bar_top:.3f}",
            "width": f"{bar_w:.3f}", "height": f"{bar_h:.3f}",
            "fill": f"url(#{grad_id})",
        },
    )
    ...
    for lat, label in _SST_LEGEND_LABELS:
        ...
        ET.SubElement(
            g, _qname("line"),
            {
                "x1": f"{tx:.3f}", "y1": f"{bar_top + bar_h:.3f}",
                "x2": f"{tx:.3f}", "y2": f"{bar_top + bar_h + label_fs * 0.4:.3f}",
                "stroke": _LEGEND_FILL, "stroke-opacity": "0.8",
                "stroke-width": "0.6",
            },
        )
        ET.SubElement(
            g, _qname("text"),
            {
                "x": f"{tx:.3f}", "y": f"{tick_y:.3f}",
                "font-family": _LEGEND_FONT, "font-size": f"{label_fs:.3f}",
                "fill": _LEGEND_FILL, "font-weight": "normal",
                "text-anchor": anchor,
            },
        ).text = label


def _append_land_legend_block(
...
def _append_currents_legend_block(
    root: ET.Element, evb: ViewBox, y: float, block_h: float
) -> None:
    label_fs  = block_h * 0.30
    ...
    ET.SubElement(g, _qname("path"),
                  {"d": d, "fill": _LEGEND_ARROW_FILL, "fill-opacity": "0.9", "stroke": "none"})
    ET.SubElement(
        g, _qname("text"),
        {
            "x": f"{cx:.3f}", "y": f"{ay + block_h * 0.33:.3f}",
            "font-family": _LEGEND_FONT, "font-size": f"{label_fs:.3f}",
            "fill": _LEGEND_FILL, "font-weight": "normal", "text-anchor": "middle",
        },
    ).text = "Meeresströmung"
...
def append_legend(
    ...
    margin_v    = legend_h * 0.10

    g_bg = ET.SubElement(root, _qname("g"), {"id": "WAZ_Legende_BG"})
    ET.SubElement(
        g_bg, _qname("rect"),
        {
            "x": f"{evb.min_x:.3f}", "y": f"{legend_top:.3f}",
            "width": f"{evb.width:.3f}", "height": f"{legend_h:.3f}",
            "fill": "#ffffff", "fill-opacity": "1",
            "stroke": "#c8c8c8", "stroke-width": "0.4",
        },
    )

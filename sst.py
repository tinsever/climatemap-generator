...
def _sst_color_vec(abs_lat_arr: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def build_sst_image(ocean: np.ndarray, raster_h: int, alpha: int=155, coast_window_px: int=180, coast_max_shift: float=5.0, stream_v: Optional[np.ndarray]=None, stream_u: Optional[np.ndarray]=None, current_max_shift: float=14.0, enclosure_max_shift: float=12.0, debug_mode: Optional[str]=None, month: int=1) -> bytes:
    from scipy.ndimage import binary_dilation, binary_erosion, label, uniform_filter1d
    h, w = ocean.shape
    ...
    land_to_east = uniform_filter1d(np.roll(land_f, -half, axis=1), size=coast_window_px, axis=1, mode='wrap')
    land_to_west = uniform_filter1d(np.roll(land_f, half, axis=1), size=coast_window_px, axis=1, mode='wrap')
    ...
    upwelling_shift = east_boundary * offshore_decay * trade_lat_w[:, np.newaxis] * upwelling_max_shift
    upwelling_shift[~ocean] = 0.0
    upwelling_shift = ndimage.gaussian_filter(upwelling_shift, sigma=(3.0, 3.0), mode=('nearest', 'wrap'))
    upwelling_shift[~ocean] = 0.0
    coastal_shift += upwelling_shift
    enclosure_window = max(45, int(coast_window_px * 0.5))
    ...
    enc_strength = np.clip((enc - 0.2) / 0.35, 0.0, 1.0)
    strait_r = 4
    pad = strait_r + 2
    o_x = np.pad(ocean, ((0, 0), (pad, pad)), mode='wrap')
    o_xy = np.pad(o_x, ((pad, pad), (0, 0)), mode='constant', constant_values=False)
    o_open = binary_dilation(binary_erosion(o_xy, iterations=strait_r), iterations=strait_r)
    ...
    if n > 0:
        ...
        semi_strength = ndimage.gaussian_filter(semi.astype(np.float32), sigma=(max(1.0, strait_r * 0.8), max(1.0, strait_r * 0.8)), mode=('nearest', 'wrap'))
    ...
    coastal_shift = coastal_shift * (1.0 - 0.9 * enc_strength2)
    if debug_mode == 'B':
        coastal_shift[:] = 0.0
    ...
    if (stream_v is not None or stream_u is not None) and debug_mode != 'A':
        ...
        for _ in range(n_steps):
            su_src = map_coordinates(su, [src_y.ravel(), src_x.ravel()], order=1, mode='nearest').reshape(h, w)
            sv_src = map_coordinates(sv, [src_y.ravel(), src_x.ravel()], order=1, mode='nearest').reshape(h, w)
            new_y = np.clip(src_y - sv_src * ds_step, 0.0, h - 1.0)
            new_x = np.clip(src_x - su_src * ds_step, 0.0, w - 1.0)
            on_land = map_coordinates(ocean_f, [new_y.ravel(), new_x.ravel()], order=0, mode='nearest').reshape(h, w) < 0.5
            frozen |= on_land
            ...
            frozen[~ocean] = True
        eff = map_coordinates(abs_lat_2d, [src_y.ravel(), src_x.ravel()], order=1, mode='nearest').reshape(h, w)
        ...
        adv_shift[~ocean] = 0.0
        adv_shift *= 1.0 - 0.6 * enc_strength2
        if np.any(ocean):
            adv_p95 = float(np.percentile(np.abs(adv_shift[ocean]), 95))
            if adv_p95 > 1e-09:
                adv_shift = np.clip(adv_shift / adv_p95 * current_max_shift, -current_max_shift, current_max_shift * 0.8)
        total_shift += adv_shift
    A_annual = 1.0 + 7.0 * np.exp(-((a_arr - 40.0) / 18.0) ** 2)
    month_norm = (month - 1) % 12
    annual_shift_1d = A_annual * math.cos(2.0 * math.pi * month_norm / 12.0) * np.sign(lat_arr)

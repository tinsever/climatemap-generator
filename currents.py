...
def coastal_damping(dist_to_land: float, scale: float) -> float:
    return math.tanh(dist_to_land / max(1e-6, scale))


def coastal_repulsion_strength(dist_to_land: float, scale: float) -> float:
    return math.exp(-dist_to_land / max(1e-6, scale))


def steer_away_from_coast(
    u: float,
    v: float,
    dist_to_land: float,
    gx: float,
    ...
    repel = COAST_REPEL_GAIN * coastal_repulsion_strength(
        dist_to_land, scale=COAST_REPEL_SCALE
    )
    u += repel * nx
    v += repel * ny
...
def western_intensification_factor(
    x: float, basin: BasinStats, strength: float, scale: float
) -> float:
    dist_from_west = x - basin.x_min
    return 1.0 + strength * math.exp(-dist_from_west / max(1e-6, scale))
...
def build_arrows(
    ...
    for y in range(spacing_px // 2, h, spacing_px):
        for x in range(spacing_px // 2, w, spacing_px):
            ...
            u, v, speed = vector_at_point(
                x, y, w, h, labels, dist, grad_x, grad_y, basins, stream_u, stream_v
            )
            if speed < 0.02:
                continue

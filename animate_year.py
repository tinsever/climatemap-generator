"""Jahresanimation: 12 Monate als MP4 exportieren."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="animate_year",
        description="Erzeugt Jahresanimationen (12 Monate) als MP4 für ausgewählte Exporte.",
    )
    parser.add_argument("input", help="Eingabe-SVG (Weltkarte)")
    parser.add_argument(
        "output",
        help="Ausgabe-Präfix. Erzeugt z.B. out/anim_klimaatlas_jahresanimation.mp4",
    )
    parser.add_argument(
        "--exports",
        default="atlas,ozean",
        metavar="KEYS",
        help=f"Kommagetrennte Exporte. Mögliche Werte: {key_help}. Standard: atlas,ozean",
    )
    parser.add_argument(
        "--no-legend",
        action="store_true",
        help="Legende nicht in die Animations-Frames einbetten (Standard: Legende sichtbar).",
    )
    parser.add_argument(
        "--fps",
        type=int,
        default=2,
        metavar="N",
        help="Frames pro Sekunde im MP4 (Standard: 2)",
    )
    parser.add_argument(
        "--raster-w",
        type=int,
        default=1800,
        metavar="PX",
        help="Rasterbreite (Standard: 1800)",
    )
    ...
    for spec in ALL_EXPORTS:
        ...
        with tempfile.TemporaryDirectory(prefix="klima_anim_") as tmpdir:
            ...
            for month in range(1, 13):
                ...
                data = build_all_layers_for_month(
                    args.input,
                    month=month,
                ...
                write_export(
                    ...
                    month_label=MONATSNAMEN[month - 1],
                )
            ...
            if shutil.which("ffmpeg"):
                ...
                subprocess.run(
                    [
                        ...
                        str(args.fps),
                        "-i",
                        os.path.join(tmpdir, "frame_%02d.png"),
                        "-c:v",
                        "libx264",
                        "-pix_fmt",
                        "yuv420p",

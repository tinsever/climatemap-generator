...
def get_viewbox(root: ET.Element) -> ViewBox:
    ...
    if vb:
        ...
        if len(parts) == 4:
            return ViewBox(min_x=float(parts[0]), min_y=float(parts[1]), width=float(parts[2]), height=float(parts[3]))
    ...
    raise ValueError('SVG braucht viewBox oder width/height, damit Koordinaten stabil sind.')
...
def _parse_rgb_color(value: str) -> Optional[Tuple[int, int, int]]:
    s = value.strip().lower()
    m = re.match('^rgba?\\(\\s*([0-9]{1,3})\\s*,\\s*([0-9]{1,3})\\s*,\\s*([0-9]{1,3})(?:\\s*,\\s*[0-9\\.]+\\s*)?\\)$', s)
    if not m:
        return None
...
def _insert_raster_layer(root: ET.Element, layer_id: str, vb: ViewBox, b64_png: str, insert_index: int) -> int:
    img = ET.Element(_qname('image'), {'id': layer_id, 'x': f'{vb.min_x:.2f}', 'y': f'{vb.min_y:.2f}', 'width': f'{vb.width:.2f}', 'height': f'{vb.height:.2f}', 'href': f'data:image/png;base64,{b64_png}', 'preserveAspectRatio': 'none'})
